...
const JobDetailPage = () => {
  ...
  const loadJob = useCallback(async () => {
    // VALIDAÇÃO: Verifica se o ID existe antes de fazer a query
    if (!id) {
      toast({ 
        title: 'Erro', 
  ...
  const handleStartChat = async () => {
    ...
    if (!job?.created_by) {
      toast({ 
        title: 'Erro', 
        description: 'Vaga sem contratante válido.', 
        variant: 'destructive' 
      });
      return;
    }
    if (job.created_by === me) {
      toast({ 
        title: 'Ops', 
        description: 'Você é o criador desta vaga.', 
        variant: 'default' 
      });
      return;
    }
    ...
    if (findErr) {
      ...
      toast({ 
        title: 'Erro', 
        description: 'Falha ao abrir o chat.', 
        variant: 'destructive' 
      });
      return;
    }
