...
import { Search, MapPin, Briefcase, Filter, DollarSign, Clock, Loader2, MessageCircle } from 'lucide-react';
...
const JobSearchPage = () => {
  ...
  const fetchJobs = async () => {
    ...
    const { data, error } = await supabase
      .from('jobs')
      // ⚠️ traz explicitamente created_by para poder abrir chat
    ...
    >
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg mb-8 border border-blue-100">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <Button
            className="w-full bg-blue-600 hover:bg-blue-700 text-white md:mt-0 mt-4"
            onClick={fetchJobs}
            disabled={loading}
          >
            {loading ? (
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredJobs.map((job) => {
            const canChat = Boolean(job.created_by); // fallback se vagas antigas não tiverem created_by
                ...
                <Card className="hover:shadow-xl transition-shadow duration-300 h-full flex flex-col">
                  ...
                  <CardContent className="space-y-2 text-sm flex-grow">
                    <p className="flex items-center">
                      <MapPin className="mr-2 h-4 w-4 text-blue-500" /> {job.location || '—'}
                    </p>
                    <p className="flex items-center">
                      <Briefcase className="mr-2 h-4 w-4 text-blue-500" />{' '}
                      {job.job_type
                        ? job.job_type.charAt(0).toUpperCase() +
                          job.job_type.slice(1).replace(/([A-Z])/g, ' $1').trim()
                        : '—'}
                    </p>
                    {job.salary && (
                      <p className="flex items-center">
                        <DollarSign className="mr-2 h-4 w-4 text-green-500" /> {job.salary}
                      </p>
                    )}
                  ...
                  <CardFooter className="flex justify-between items-center text-xs text-gray-500 pt-4">
                    <p className="flex items-center">
                      ...
                      Postado em:{' '}
                      {job.date_posted
                        ? new Date(job.date_posted).toLocaleDateString('pt-BR')
                        : '—'}
                    </p>

                    {/* 👉 “Ver Detalhes” agora abre o CHAT com o criador da vaga */}
                    <div className="flex items-center gap-3">
                      <Button variant="link" asChild className="text-blue-600 hover:text-blue-800 p-0 h-auto">
                        <Link
                          to={
                            canChat
                              ? `/chat/${job.created_by}`
                              : `/vaga/${job.id}` // fallback se não houver created_by
                          }
                          state={detailsState}
                        >
                      ...
                      {/* Se quiser manter um link explícito para a página de detalhes, descomente: */}
                      {/* <Button variant="ghost" asChild className="h-7 px-2">
                        <Link to={`/vaga/${job.id}`} state={{ job }}>Página</Link>
                      </Button> */}
                    </div>
                  </CardFooter>
                </Card>
        ...
        <div className="text-center py-10">
          <img
            className="mx-auto mb-4 w-48 h-48 opacity-70"
            alt="Nenhuma vaga encontrada"
            src="https://images.unsplash.com/photo-1682624400764-d2c9eaeae972"
          />
